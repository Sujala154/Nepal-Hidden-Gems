...
async function seedDestinations() {
  try {
    ...
    // Find a contributor to assign as creator
    const contributor = await User.findOne({ role: 'contributor' });
    if (!contributor) {
      console.warn('⚠️ No contributor found to assign as creator. Skipping seed.');
      return;
    }
